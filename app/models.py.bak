...
class Product(Base):
    ...
    # Медиа
    images = Column(JSON, default=list)          # ["path/to/img1.jpg", ...]
    video_url = Column(String(500), default="")  # ссылка на RuTube
    # Характеристики
    specs = Column(JSON, default=dict)           # {"Разрешение": "4K", ...}
    # SEO
    seo_title = Column(String(300), default="")
    seo_description = Column(String(500), default="")
    # Парсинг
...
class Order(Base):
    ...
    items = Column(JSON, default=list)  # [{"id": 1, "name": "...", "price": 0, "qty": 1}]
    total = Column(Float, default=0.0)
    status = Column(String(50), default="new")  # new, processing, done, cancelled
    created_at = Column(DateTime(timezone=True), server_default=func.now())
